    ...
    items: [
      ...
      },
      {
        ...
        children: [
          ...
          {id: 'gallery', label: 'Gallery', labelAr: 'المعرض', path: 'gallery'},
...
export async function getNavigation() {
  return ensureDocument('navigation', buildNavigationSeed);
}
...
async function seedPageCopy() {
  const pageIds = ['about', 'contact', 'careers', 'downloads', 'terms', 'privacy'];
  const pages = {};
...
export async function getPageCopy(pageId) {
  ...
  if (pageId) {
    return doc.pages?.[pageId] || defaultPageCopy(pageId);
  }
  return doc;
}
...
export async function savePageCopy(pageId, patch = {}) {
  ...
  const doc = await ensureDocument('page-copy', seedPageCopy);
  const current = doc.pages?.[pageId] || defaultPageCopy(pageId);
  const nextPage = {...current, ...patch, pageId, updatedAt: nowIso()};
  const next = {
    ...doc,
