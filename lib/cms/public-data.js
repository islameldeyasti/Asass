...
export async function getPublicVideos() {
  try {
    const {getVideos} = await import('./content-service');
    const items = await getVideos();
    if (items?.length) {
      const {sortVideos} = await import('@/data/videos');
      return sortVideos(notDraft(items));
    }
  } catch {
    /* fall through */
  }
  const {videoItems, sortVideos} = await import('@/data/videos');
  return sortVideos(videoItems.filter((item) => item.status !== 'draft'));
}
