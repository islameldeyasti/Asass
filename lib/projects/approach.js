...
export function resolveProjectApproach(project, locale = 'en') {
  const ar = locale === 'ar';
  const customLead = cleanText(
    ar ? project?.approachAr || project?.approach : project?.approach,
  );
  const customSupport = cleanText(
  ...
  const title =
    cleanText(ar ? project?.titleAr || project?.title : project?.title || project?.titleAr) ||
    (ar ? 'هذا المشروع' : 'this project');
  ...
  const kind = cleanText(project?.kind) || 'built';
  ...
  const writeLead = ar ? writer?.ar : writer?.en;
  const lead =
    typeof writeLead === 'function'
      ? writeLead(ctx)
      : (ar ? DEFAULT_APPROACH.ar : DEFAULT_APPROACH.en)(ctx);
  ...
  return {lead: cleanText(lead) || (ar ? DEFAULT_APPROACH.ar : DEFAULT_APPROACH.en)(ctx), support};
}
